...
then
  ...
  mkdir $base/htdocs/modules/custom/$project_machine
  mv default.module $base/htdocs/modules/custom/$project_machine/$project_machine.module
  mv default.info $base/htdocs/modules/custom/$project_machine/$project_machine.info
  sed -i '' s/default/$project_machine/g $base/htdocs/modules/custom/$project_machine/$project_machine.*
  echo "*****************************************"
...
if [[ -z `grep "# if (file_exists(__DIR__ . '/settings.local.php')) {" $base/htdocs/sites/default/settings.php` ]]
then
  echo "Enabling local development override configuration.";
  sed -i '' "s/# if (file_exists(__DIR__ . '\/settings.local.php')) {/if (file_exists(__DIR__ . '\/settings.local.php')) {" $base/htdocs/sites/default/settings.php
  sed -i '' "s/#   include __DIR__ . '\/settings.local.php';/   include __DIR__ . '/settings.local.php';" $base/htdocs/sites/default/settings.php
  sed -i '' "s/# }/ }" $base/htdocs/sites/default/settings.php
fi

if [[ -f $base/htdocs/sites/default/settings.local.php ]]
then
  if [[ ! -z `grep "$databases" $base/htdocs/sites/default/settings.local.php` ]]
  then
    echo "Adding database settings defaults to settings.local.php";
    cat $base/cnf/default.settings.local.addendum >> $base/htdocs/sites/default/settings.local.php
  fi
